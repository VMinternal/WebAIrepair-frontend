...
export default function CustomerPortalPage() {
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage('🎉 Đặt lịch thành công! Hệ thống đã ghi nhận.');
        setCustomerName('');
        setPhone('');
        setDeviceModel('');
        setDescription('');
        ...
        setMessage(`❌ Thất bại: ${data.message || 'Lỗi hệ thống'}`);
      }
    } catch (error) {
      setMessage('❌ Không thể kết nối đến máy chủ Backend (Cổng 8000)!');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="relative min-h-screen flex flex-col bg-slate-950 text-slate-100">
      ...
      {/* Header điều hướng quay lại trang chủ trung tâm */}
            ...
            <div className="flex-grow p-6 space-y-4 overflow-y-auto text-sm">
              <div className="flex gap-3 max-w-[85%]">
                ...
                <div className="bg-slate-800/80 text-slate-200 p-3.5 rounded-2xl border border-slate-700/40 shadow-sm">
                  Xin chào quý khách! Mình là trợ lý AI của **WebAI Repair**. Thiết bị của bạn đang gặp lỗi gì thế?
                </div>
              </div>
            </div>

            <div className="p-4 border-t border-slate-800/80 bg-slate-950/40 rounded-b-3xl flex gap-2">
              <input type="text" placeholder="Nhập tình trạng lỗi..." className="flex-grow px-4 py-3 bg-slate-950 border border-slate-800/80 rounded-xl focus:outline-none focus:border-emerald-500/80 text-sm text-white placeholder-slate-600 transition" />
            ...
            {!showBookingForm ? (
              <div className="space-y-8 lg:pr-6">
                <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-3.5 py-1.5 rounded-full text-xs text-emerald-400 font-semibold">
                  <span className="w-2 h-2 bg-emerald-400 rounded-full animate-ping" /> Portal Khách Hàng
                </div>
                <h1 className="text-4xl md:text-5xl font-black leading-[1.1] text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-100 to-slate-400">
                  Chẩn đoán bằng AI. <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">Sửa chữa thần tốc.</span>
                </h1>
                <p className="text-slate-400 text-base">Đặt trước lịch hẹn sửa chữa chỉ trong vài giây. Tiết kiệm thời gian, minh bạch chi phí.</p>
              ...
              <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl relative">
                ...
                <h3 className="text-2xl font-black text-white">Đăng Ký Đặt Lịch</h3>
                <form onSubmit={handleBookingSubmit} className="space-y-5 mt-6">
                  ...
                  <input type="text" required placeholder="Tên thiết bị & Dòng máy" value={deviceModel} onChange={(e) => setDeviceModel(e.target.value)} className="w-full p-3.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-emerald-500" />
                  <textarea required placeholder="Hiện tượng lỗi..." value={description} onChange={(e) => setDescription(e.target.value)} className="w-full p-3.5 bg-slate-950 border border-slate-800 rounded-xl text-white focus:outline-none focus:border-emerald-500" rows={3} />
                  <button type="submit" disabled={loading} className="w-full py-4 bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 font-black rounded-xl hover:from-emerald-400 hover:to-teal-400 transition text-sm tracking-wider uppercase">{loading ? 'Đang gửi...' : 'Gửi Yêu Cầu Đặt Lịch'}</button>
                </form>
