  ...
  return (
    <div className="max-w-7xl mx-auto p-6 text-slate-100 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">Post Management</h1>
          <p className="text-sm text-slate-400 mt-1">Manage blog articles, status, and SEO content</p>
        </div>
        <button
          onClick={() => {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {loading ? (
                <tr>
                  <td colSpan={5} className="text-center py-12 text-slate-400">Loading data...</td>
                </tr>
              ) : posts.length === 0 ? (
                <tr>
                  <td colSpan={5} className="text-center py-12 text-slate-500">No posts found.</td>
                </tr>
              ) : (
                posts.map((post) => (
                        ...
                        <div className="flex flex-col">
                          ...
                          {typeof post.author === 'object' && post.author?.role && (
                            <span className="mt-1 inline-flex items-center">
                              ...
                              >
                                {post.author.role.toUpperCase() === 'ADMIN' ? '👑 Admin' : '🛠️ Tech'}
                              </span>
                            </span>
                          )}
                        </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white rounded-xl text-sm font-medium transition shadow-lg shadow-rose-600/20"
              >
                Agree to delete
              </button>
            </div>
