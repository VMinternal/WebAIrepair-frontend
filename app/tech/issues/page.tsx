...
import { useTechIssues } from './useTechIssues';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Search issues using AI Vector Search or manage problem records
          </p>
        </div>
        <button
          type="button"
          onClick={openCreateModal}
          className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white font-medium text-sm rounded-xl shadow-lg shadow-indigo-600/20 transition-all cursor-pointer"
        >
      ...
      <div className="bg-slate-900/80 border border-slate-800 p-2 rounded-2xl shadow-xl backdrop-blur-xl">
        <form onSubmit={handleAiSearch} className="flex flex-col sm:flex-row gap-2">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Describe symptoms or issue (e.g., system failing to post under load, overheating)..."
              value={searchQuery}
          ...
          <button
            ...
            className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-medium text-sm rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/20 transition-all whitespace-nowrap cursor-pointer"
          >
            🤖 Search using AI
          </button>
          {isAiSearching && (
            <button
              ...
              className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 font-medium text-sm rounded-xl transition-all whitespace-nowrap cursor-pointer"
            >
              Reset Search
            </button>
          )}
          ...
          <table className="w-full text-left text-sm text-slate-300">
            <thead className="bg-slate-950/50 text-slate-400 border-b border-slate-800/80 text-xs uppercase tracking-wider font-semibold">
              <tr>
                ...
                <th className="px-6 py-4">Device / System</th>
                <th className="px-6 py-4">Related Components</th>
                {isAiSearching && <th className="px-6 py-4">AI Similarity</th>}
                    ...
                    <td className="px-6 py-4 max-w-xs">
                      <div className="text-xs text-slate-300 line-clamp-2">
                        {Array.isArray(issue.symptoms) && issue.symptoms.length > 0 ? (
                          issue.symptoms
                            .map((s: any) =>
                              typeof s === 'object' && s !== null ? s.content || '' : String(s)
                            )
                            .filter(Boolean)
                            .join(', ')
                    ...
                    <td className="px-6 py-4 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="px-3 py-1.5 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 border border-indigo-500/20 rounded-xl text-xs font-medium transition cursor-pointer"
                        >
                          Edit
                        </button>
                        <button
                          ...
                          className="px-3 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 rounded-xl text-xs font-medium transition cursor-pointer"
                        >
                          Delete
                        </button>
                      </div>
        ...
        <div className="flex items-center justify-between text-sm text-slate-400 px-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3.5 py-1.5 bg-slate-900 border border-slate-800 rounded-xl hover:bg-slate-800 disabled:opacity-40 transition cursor-pointer disabled:cursor-not-allowed"
            >
              Previous
            </button>
            <button
